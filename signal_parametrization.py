...
def magspec(framed, ndft) :
    ...
    if np.shape(framed)[1] > ndft:
        logging.warning('frame length (%d) is greater than DFT size (%d), frame will be truncated. Increase ndft to avoid.',
...
def melfreqwrap(magspectrum, rate, ndft,  lowfreq, highfreq, nmfilter):
    '''Function creates a filter bank for MFCC features and returnes the dot of magspectrum and mel filter bank
    magspectrum: array of the power spectrum
    rate: signal rate in Hz
    ndft: length of DFT,It can't be grater than length of frame
    lowfreq: lowest frequency of Mel Bank Filter in Hz
    highfreq: highest frequency of Mel Bank Filter, in Hz, the highest is rate/2
...
def centerfreq(f, j, a, b, c):
    ...
    :param a: numerical value. For ERB in polynominal format a = 6.23 * 10**(-6) , ERB = a* f^2 + b*f + c
    :param b: numerical value. For ERB in polynominal format b = 93.39 * 10**(-3), ERB = a* f^2 + b*f + c
    :param c: numerical value. For ERB in polynominal format c = 28.52, ERB = a* f^2 + b*f + c
    :return:
    '''
    a__ = (-1) ** j * (0.5/(700+f))
    b__ = (-1) ** j * (700/(700+f))
...
def hmelfreqwrap(magspectrum, rate, ndft,  lowfreq, highfreq, nmfilter):
    ...
    Function creates a filter bank for HFCC features and returnes the dot of magspectrum and mel filter bank
    magspectrum: array of the power spectrum
    rate: signal rate in Hz
    ndft: length of DFT,It can't be grater than length of frame
    lowfreq: lowest frequency of Mel Bank Filter in Hz
    ...
    for i in range(0, nmfilter+1):
        ...
        flowlim[i] = -(700 + ERB) + m.sqrt((700 +ERB) ** 2 + melfreq[i]*(melfreq[i] + 1400))
...
def mfcc(signal, rate = 41000, winlen=0.01, winstep=0.005, winfunc = np.hamming, ndft=512, lowfreq = 0,
         highfreq = 20500, nmfilter = 41, ncep = 13, Energy = True, pre_emphasis = 0.95, cep_lifter= 22):
    '''
    ...
    highfreq: highest frequency of Mel Bank Filter, the highest is rate/2 in Hz, default is 20,5 kHz
    nmfilter: Number of filters in Mel Bank Filter, default is 41
    ncep: Number of coefficients, default is 13
    Energy: if this is true, the zeroth cepstral coefficient the log of the total frame energy, default is True
    pre_emphasis : coefficient of pre emphasis filter, 0 is no filter, default is 0.95
    ...
    melspectrum = melfreqwrap(magspectrum, rate, ndft, lowfreq, highfreq, nmfilter)
    melspectrum = np.where(melspectrum == 0, np.finfo(float).epsneg, melspectrum)
...
def hfcc(signal, rate = 41000, winlen=0.01, winstep=0.005, winfunc = np.hamming, ndft=512, lowfreq = 0,
         highfreq = 20500, nmfilter = 41, ncep = 13, Energy = True, pre_emphasis = 0.95, cep_lifter= 22):
    '''
    ...
    magspectrum = magspec(framed, ndft)
    assert highfreq <= rate / 2, "highfreq is greater than samplerate/2"
    hmelspectrum = hmelfreqwrap(magspectrum, rate, ndft, lowfreq, highfreq, nmfilter)
    hmelspectrum = np.where(hmelspectrum == 0, np.finfo(float).epsneg, hmelspectrum)
    # melspectrum can not be zero, problems with log
    melspectrum = np.log(hmelspectrum)
    hfcc_ = sc.dct(hmelspectrum, type=2, axis=1, norm='ortho')[:, :ncep]
    if Energy:
        energy = np.sum(magspectrum, 1)
        energy = np.where(energy == 0, np.finfo(float).epsneg, energy)
        # energy can not be zero, problems with log
