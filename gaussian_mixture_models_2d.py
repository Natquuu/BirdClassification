# -*- coding: utf-8 -*-
"""Gaussian_Mixture_Models_2D.ipynb
...
for step in range(40):
    ...
    if step % 1 == 0:
        ...
        likelihood = []
        for j in range(k):
            likelihood.append(multivariate_normal.pdf(
                x=pos, mean=means[j], cov=cov[j]))
    ...
    for j in range(k):
        ...
        means[j] = np.sum(b[j].reshape(len(X), 1) * X,
                          axis=0) / (np.sum(b[j]+eps))
        cov[j] = np.dot((b[j].reshape(len(X), 1) * (X - means[j])
                         ).T, (X - means[j])) / (np.sum(b[j])+eps)
